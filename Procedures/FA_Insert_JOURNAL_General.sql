create or replace 
procedure FA_Insert_JOURNAL_General
     (
...
--	  BreakPoints,2022
--versions No.	   : 1
--Program name     : FA_Insert_JOURNAL_General
...
--Description	   : This procedures inserts JOURNAL                                    
--Program Inputs   : 
...
where INV_CUSTOMER_ID=pc_CUSTOMER_ID and INV_ID=pc_invSer
 and INV_GUIDANCE_TYPE=4
 group by INV_DETAIL_TAX_PERCENTAGE;
 
 
begin

if pc_GUIDANCE_TYPE=4 then
...
INTO FA_JOURNAL
  ...
  (
    ...
    pc_CUSTOMER_ID,
    trunc(to_date(sysdate,'dd/mm/rrrr')),
    ' قيد مبيعات - إثبات تسليم - '||pc_Sale_Desc,
     pc_JOR_INVOICE_ID,
     trunc(to_date(pc_JOR_INVOICE_DATE,'dd/mm/rrrr')),
     pc_JOR_ATTACHMENT,
     pc_pay_Type
...
INTO FA_JOURNAL_DETAILS
  ...
  commit;
   ...
   ------------------------------------------قيد الجانب الدائن لتسليم المبيعات ضريبي-------------------------------
...
INTO FA_JOURNAL_DETAILS
  ...
  (
   ...
   -------------------------------------------------------إنهاء قيد الجانب الدائن لتسليم المبيعات ضريبي-----------------------------
